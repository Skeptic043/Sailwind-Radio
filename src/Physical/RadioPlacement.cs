...
namespace SailwindRadio.Physical
{
    internal static class RadioPlacement
    {
        ...
        {
            Vector3 center = position + rotation * new Vector3(0, .19f, 0);
            ...
            if (model)
            {
                ...
                if (!ClearVolumeAndPath(walkCenter, walkRotation, walkStart, null)) return false;
            }
            return ClearVolumeAndPath(center, rotation, start, coarseHull);
        }

        private static bool UnitScale(Transform transform) => (transform.lossyScale - Vector3.one).sqrMagnitude < .0001f;

        private static bool ClearVolumeAndPath(Vector3 center, Quaternion rotation, Vector3 start, Collider allowedHull)
        {
